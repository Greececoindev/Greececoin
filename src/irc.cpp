...
// Copyright (c) 2011-2012 GreeceCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("GreeceCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #GreeceCoinTEST0\r");
            Send(hSocket, "WHO #GreeceCoinTEST0\r");
        } else {
            // randomly join #GreeceCoin00-#GreeceCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #GreeceCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #GreeceCoin%02d\r", channel_number).c_str());
        }
